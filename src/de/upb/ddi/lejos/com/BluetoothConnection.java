...
/**
 * Bluetooth Verbindung zwischen NXT Bausteinen. Stellt Verbindungen zwischen
 * Bausteinen her und Methoden zum Versandt von Daten bereit. Für weitere
 * Informationen siehe {@link RemoteConnection}.
 * 
...
public class BluetoothConnection implements RemoteConnection {
	...
	/**
	 * Timeout für Inquire-Vorgänge in 1.28 Sekunden (
	 * <code>INQUIRE_TIMEOUT * 1.28</code>).
	 */
	private static final int INQUIRE_TIMEOUT = 5;

	/**
	 * Timeout beim disconnect damit die Streams erfolgreich geschlossen werden
	 * in Millisekunden.
	 */
	...
	/**
	 * Konstruktor mit automatischer Bestimmung der Bluetooth-Adresse. Die
	 * Aufrufe von {@link Bluetooth#getFriendlyName()} und
	 * {@link Bluetooth#getLocalAddress()} können teils einige Sekunden in
	 * Anspruch nehmen. Der alternative Konstruktor {@link
	 * BluetoothConnection(String, String)} kann dies beschleunigen.
	 * <p/>
	...
	/**
	 * Alternativer Konstruktor mit manueller Zuweisung der Adresse. Im
	 * Gegensatz zum Standard-Konstruktor nimmt dieser Werte für Bluetooth-Name
	 * und -Adresse entgegen. Dies kann den Startvorgang deutlich beschleunigen,
	 * schränkt aber die Flexibilität des Programms ein auf verschiedenen NXTs
	 * zu laufen, denn Name und Adresse müssen mit denen des NXTs
	 * übereinstimmen.
	 * <p/>
	...
	public void connect( int timeout ) {
		...
		this.connection = Bluetooth.waitForConnection(timeout,
	 ...
	 * passenden Eintrag. Die Identifizierung kann eine Bluetooth-Adresse oder
	 * ein Gerätename sein.
	 * <p/>
	...
	/**
	 * Der Inquire-Prozess versucht unbekannte Bluetooth Geräte zu finden und zu
	 * pairen (der Liste der bekannten Geräte hinzuzufügen).
	 * 
	 * @todo Findet momentan nur andere NXTs und keine PCs und Smartphones. (->
	 *       Anpassen der cod)
	 * @param identifier Name oder Adresse des anderen Geräts
	 * @return Das Gerät als <code>RemoteDevice</code>, falls es gefunden wurde.
	 *         Sonst <code>NULL</code>.
	...
	protected RemoteDevice inquire( String identifier ) {
		...
		ArrayList<RemoteDevice> inquireDevices = Bluetooth.inquire(10,
				INQUIRE_TIMEOUT, 0x84);
		for( int i = 0; i < inquireDevices.size(); i++ ) {
			RemoteDevice btrd = inquireDevices.get(i);
